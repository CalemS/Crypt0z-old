...
#if defined(NDEBUG)
# error "Crypt0z cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9895467990ef6fb46247869da32e8d0f3815e71e83d8d61512070685f4fb4037");
...
const string strMessageMagic = "Crypt0z Signed Message:\n";
...
{
    // Crypt0z: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Crypt0z: 1 days
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Crypt0z: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Crypt0z: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0f52859db2a8014cb5eee2f35600439a9c645e5934ddf1b6e5d6568715395781");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "bitcoin-abc-developers-address-a-vulnerability-found";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0415139e65bdf543c33ecaa4be1857388fd3c39609bd8b8a50e4beeefae6e0631f8eb89c27da9975f6e476b1fa823e2e2016769ac633a0b0618b781902d921e290") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1525693015;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085309001;

        if (fTestNet)
        {
            block.nTime    = 1525692997;
            block.nNonce   = 386941713;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x523ea22bf5ba29588b4aa47289e1298e6c709c192bf22c29032e7bd878436f82"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb5, 0xdd }; // Crypt0z: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("Crypt0zMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("Crypt0zMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static Crypt0zMiner(CWallet *pwallet)
{
    printf("Crypt0zMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("crypt0z-miner");
    ...
    try { loop {
        ...
        printf("Running Crypt0zMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("Crypt0zMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&Crypt0zMiner, pwallet));
}
